...
// Add a favourite
router.post('/:userName/favourites', asyncHandler(async (req, res, next) => {
  // find movie and find user
  ...
  if (user.favourites.includes(movie._id)) {
    res.status(401).json({success: false, msg: 'Movie already in favourites list'});
    return next();
  }
